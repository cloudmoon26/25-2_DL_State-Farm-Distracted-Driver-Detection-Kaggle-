...
def train_one_fold(fold_idx, train_df, val_df):
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            best_model_wts = copy.deepcopy(model.state_dict())
            torch.save(best_model_wts, f'alexnet_fold{fold_idx}.pth')
...
with torch.no_grad():
    for i in tqdm(range(0, len(img_names), BATCH_SIZE), desc="Predicting with TTA"):
        ...
        batch_center = torch.stack([transform_centercrop(img) for img in pil_images]).to(DEVICE)
        batch_resize = torch.stack([transform_resize(img) for img in pil_images]).to(DEVICE)
        batch_5crop = torch.stack([transform_fivecrop(img) for img in pil_images]).to(DEVICE)
...
save_filename = "alexnet_tta_25_ensemble_final.csv"
